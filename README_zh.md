...
我们引入了稳健评测框架，结合 **MMS-FA** 做对齐、结合 **wespeaker** 做说话人嵌入抽取，以保证说话人归属评估更准确。

- **方法**：基于强制对齐的分段 + 基于相似度的说话人验证。
- **指标**：
  - **说话人归属准确率（ACC）**
  - **说话人相似度（SIM）**
  - **词错误率（WER）**，使用 **Whisper-large-v3** 计算。
- **数据集**：100 条中英文多轮对话，时长覆盖 30s–720s，场景包括播客、影视配音、相声等。评测代码与数据即将开源。
